...
import com.financeAndMoney.base.legacy.unzip
import com.financeAndMoney.base.legacy.zip
...
class BackingUpDeitaUseCase @Inject constructor(
    ...
    private suspend fun extractAndReadBackupZip(
        ...
        val folderName = "backingUp" + System.currentTimeMillis()
    ...
    suspend fun importJson(
        ...
        val mylonWalletCompleteData = modifiedJsonString?.let {
            json.decodeFromString<MylonWalletCompleteData>(it)
        } ?: error("Failed to parse backingUp JSON.")

        onProgress(0.4)
        insertDataToDb(completeData = mylonWalletCompleteData, onProgress = onProgress)
        onProgress(1.0)
        ...
        return ImportingResults(
            rowsFound = mylonWalletCompleteData.transactions.size,
            transactionsImported = mylonWalletCompleteData.transactions.size,
            accountsImported = mylonWalletCompleteData.accounts.size,
            categoriesImported = mylonWalletCompleteData.categories.size,
            failedRows = persistentListOf()
        )
    }
    ...
    private suspend fun accommodateExistingAccountsAndCategories(jsonString: String?): String? {
        ...
        val mylonWalletCompleteData = json.decodeFromString<MylonWalletCompleteData>(jsonString)
        val replacementPairs = getReplacementPairs(mylonWalletCompleteData)
    ...
    /** This is used to replace account & category Ids in backingUp data with existing Ids
