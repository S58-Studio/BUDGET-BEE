...
import com.oneSaver.legacy.legacyOld.ui.theme.components.BudgetBattery
...
private fun Preview_Empty() {
    com.oneSaver.legacy.MySavePreview {
        UI(
            ...
            state = BajetiSkriniState(
                timeRange = com.oneSaver.legacy.data.model.TimePeriod.currentMonth(
                    startDayOfMonth = 1
                ).toRange(1), // preview
                baseCurrency = "BGN",
...
private fun Preview_Budgets(theme: Theme) {
    com.oneSaver.legacy.MySavePreview(theme) {
        UI(
            activity = FakeActivity(),
            state = BajetiSkriniState(
