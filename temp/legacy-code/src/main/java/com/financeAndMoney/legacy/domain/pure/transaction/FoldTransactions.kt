...
import com.financeAndMoney.expenseAndBudgetPlanner.domain.pure.util.mapIndexedNel
import com.financeAndMoney.expenseAndBudgetPlanner.domain.pure.util.mapIndexedNelSuspend
import com.financeAndMoney.expenseAndBudgetPlanner.domain.pure.util.nonEmptyListOfZeros
...
object LegacyFoldTransactions {
    ...
    suspend fun <Arg> foldTransactionsSuspend(
        ...
        valueFunctions: NonEmptyList<suspend (com.financeAndMoney.base.legacy.Transaction, Arg) -> BigDecimal>,
        arg: Arg
    ): NonEmptyList<BigDecimal> = sumTransactionsSuspendInternal(
        transactions = transactions,
        valueFunctions = valueFunctions,
    ...
    internal tailrec suspend fun <A> sumTransactionsSuspendInternal(
        transactions: List<com.financeAndMoney.base.legacy.Transaction>,
        valueFunctionArgument: A,
        valueFunctions: NonEmptyList<suspend (com.financeAndMoney.base.legacy.Transaction, A) -> BigDecimal>,
        sum: NonEmptyList<BigDecimal> = nonEmptyListOfZeros(n = valueFunctions.size)
    ): NonEmptyList<BigDecimal> {
        return if (transactions.isEmpty()) {
            sum
    ...
    suspend fun <A> sumTrns(
        transactions: List<com.financeAndMoney.base.legacy.Transaction>,
        valueFunction: suspend (com.financeAndMoney.base.legacy.Transaction, A) -> BigDecimal,
        argument: A
    ): BigDecimal {
        return sumTransactionsSuspendInternal(
            transactions = transactions,
