...
fun Double.format(currencyCode: String): String {
    return this.format(MysaveCurrency.fromCode(currencyCode))
}

fun Double.format(currency: MysaveCurrency?): String {
...
fun decimalPartFormatted(currency: String, value: Double): String {
    return if (MysaveCurrency.fromCode(currency)?.isCrypto == true) {
...
fun formatInputAmount(
    ...
    val decimalCountOkay = MysaveCurrency.fromCode(currency)?.isCrypto == true ||
